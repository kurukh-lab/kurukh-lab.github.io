...
const WordCard = ({ word, compact = false, searchTerm = '' }: WordCardProps) => {
  if (!word) return null;
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <h3 className="text-lg font-semibold text-primary">
              {searchTerm
                ? highlightText(word.kurukh_word, searchTerm)
                : word.kurukh_word}
            </h3>
    ...
    <div className={`card bg-base-100 shadow-xl ${compact ? 'card-compact' : ''}`}>
      <div className="card-body">
        <div className="flex justify-between items-start">
          ...
          <Link to={`/word/${word.id}`} className="btn btn-xs btn-ghost">
            View Details
          </Link>
        </div>
