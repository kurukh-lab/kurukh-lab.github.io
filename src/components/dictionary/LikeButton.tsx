...
export interface LikeButtonProps {
  word: Pick<Word, 'id' | 'kurukh_word' | 'likesCount'> | null | undefined;
  size?: LikeButtonSize;
  showCount?: boolean;
}

const sizeClasses: Record<LikeButtonSize, string> = {
  sm: 'btn-sm text-xs',
  md: 'btn-sm text-sm',
  lg: 'btn-md text-base',
};

const iconSizes: Record<LikeButtonSize, string> = {
  sm: 'h-3 w-3',
  md: 'h-4 w-4',
  lg: 'h-5 w-5',
};

const LikeButton = ({ word, size = 'md', showCount = true }: LikeButtonProps) => {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      >
        ...
        {showCount && <span className="ml-1 font-medium">{likeCount}</span>}
      </button>
