...
async function callAdminTrigger(fnName: string): Promise<Record<string, unknown>> {
...
function DailyScheduleResult({ result }: { result: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-3 pt-3 border-t border-base-200 space-y-1">
      <p className="text-xs text-gray-500 mb-2">Date: {result.date as string}</p>
      {pipelines.map((p) => (
        <div key={p.name} className="flex items-center justify-between text-xs">
          ...
          <div className="flex items-center gap-2">
            ...
            {p.success ? (
              ...
              <span className="badge badge-error badge-xs" title={p.error}>
                fail
              </span>
            )}
          </div>
  ...
  return (
    <div className="mt-3 pt-3 border-t border-base-200 space-y-1">
      ...
      <p className="text-xs text-gray-600">
        Recent words:{' '}
        <span className="font-semibold">{data.recentWordsCount as number}</span>
      </p>
      {(data.wordOfTheDay as string | undefined) && (
        <p className="text-xs text-gray-600">
          Word of the day:{' '}
          <span className="font-semibold font-mono">{data.wordOfTheDay as string}</span>
        </p>
      )}
    </div>
  );
...
function WotdResult({ result }: { result: Record<string, unknown> }) {
  return (
    <div className="mt-3 pt-3 border-t border-base-200 space-y-1">
      <p className="text-xs text-gray-500">Date: {result.date as string}</p>
      {result.word ? (
        <p className="text-xs text-gray-600">
          Word:{' '}
          <span className="font-semibold font-mono">{result.word as string}</span>
        </p>
      ) : (
        <p className="text-xs text-gray-400 italic">No approved words available</p>
      )}
    </div>
  );
}
...
export default function FunctionTriggers() {
  ...
  const handleTrigger = async (id: string, fnName: string) => {
    ...
    try {
      ...
      setStates((prev) => ({
        ...prev,
        [id]: { status: 'success', result, triggeredAt: new Date().toISOString() },
      }));
    } catch (err) {
      setStates((prev) => ({
        ...prev,
  ...
  return (
    <div className="p-6">
      <p className="text-sm text-gray-500 mb-6">
        Manually trigger Cloud Functions that normally run on a schedule. Each call uses
        your current session token for admin verification.
      </p>
            ...
            >
              <div className="card-body p-5 flex flex-col">
                <div className="flex items-start justify-between mb-1">
                  <h3 className="card-title text-base">{trigger.name}</h3>
                  {state.status === 'success' && (
                    <span className="badge badge-success badge-sm shrink-0">Done</span>
                  )}
                  {state.status === 'error' && (
                    <span className="badge badge-error badge-sm shrink-0">Error</span>
                  )}
                </div>

                <p className="text-sm text-gray-500 mb-3 flex-1">{trigger.description}</p>
                ...
                <button
                  className={`btn btn-sm w-full ${
                    state.status === 'error' ? 'btn-outline btn-error' : 'btn-primary'
                  }`}
                  onClick={() => handleTrigger(trigger.id, trigger.fnName)}
                  disabled={isLoading}
                >
                  {isLoading ? (
                    <>
                ...
                {state.triggeredAt && (
                  <p className="text-xs text-gray-400 mt-2 text-right">
                    {new Date(state.triggeredAt).toLocaleTimeString()}
                  </p>
                )}
                ...
                {state.status === 'success' &&
                  state.result &&
                  RESULT_RENDERERS[trigger.fnName]?.(state.result)}
              </div>
            </div>
          );
        })}
