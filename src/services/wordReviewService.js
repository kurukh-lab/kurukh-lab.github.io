...
export const wordReviewService = {
    ...
    updateWordStatus: async (wordId, state, context = {}) => {
        ...
        // Always include vote counts when context has them (tracks voting history across all states)
    ...
    transitionWord: async (
        ...
        try {
            ...
            const { state, context } =
                await wordReviewService.loadWordStatus(wordId);
        ...
        const unsubscribe = onSnapshot(
            ...
            (docSnapshot) => {
                if (!docSnapshot.exists()) {
                    console.error(
                        `Word with ID ${wordId} not found in Firestore`,
                    );
                    return;
                }
                ...
                const context = {
                    ...
                    communityVotes: {
                        for:
                            wordData.community_votes_for ||
                            wordData.votes_for ||
                            0,
                        against:
                            wordData.community_votes_against ||
                            wordData.votes_against ||
                            0,
                    },
                    reviewedBy: wordData.reviewed_by || [],
                    history: wordData.status_history || [],
