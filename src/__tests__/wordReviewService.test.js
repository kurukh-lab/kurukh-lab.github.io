...
describe("Word Review Service", () => {
        ...
        mockWordSnapshot = {
            ...
            data: () => ({
                ...
                status_history: [
                    {
                        action: "submitted",
                        timestamp: { seconds: 1621234567, nanoseconds: 0 },
                    },
                    {
    ...
    describe("loadWordStatus", () => {
        it("should load word status and map to machine state correctly", async () => {
            // Set up specific status for this test (capture original to avoid infinite recursion)
            const originalData = mockWordSnapshot.data;
            ...
            const result =
                await wordReviewService.loadWordStatus("test-word-id");
    ...
    describe("updateWordStatus", () => {
        it("should update word status in Firestore correctly", async () => {
            ...
            await wordReviewService.updateWordStatus(
                "test-word-id",
                state,
                context,
            );
    ...
    describe("transitionWord", () => {
        it("should transition a word from pending_review to approved", async () => {
            ...
            const historyEntry =
                updateCall.history[updateCall.history.length - 1];
