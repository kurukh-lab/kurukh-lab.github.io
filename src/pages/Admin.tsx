...
const Admin = () => {
  ...
  const [currentAction, setCurrentAction] = useState<{ wordId: string; action: ActionKind } | null>(null);
  ...
  useEffect(() => {
    const fetchWordReports = async () => {
      if (!currentUser || rolesLoading || !isAdmin || activeTab !== 'reports') return;
      ...
      try {
        ...
        for (const docSnapshot of querySnapshot.docs) {
          const reportData = { id: docSnapshot.id, ...docSnapshot.data() } as ReportWithWord;
          try {
            const wordDoc = await getDoc(doc(db, 'words', reportData.word_id));
            if (wordDoc.exists()) {
              reportData.word = { id: wordDoc.id, ...wordDoc.data() } as Word;
            }
          } catch (wordErr) {
  ...
  useEffect(() => {
    const fetchCorrections = async () => {
      if (!currentUser || rolesLoading || !isAdmin || activeTab !== 'corrections') return;
  ...
  const submitAction = async () => {
    ...
    try {
      ...
      if (!result.success) throw new Error(result.error || `Failed to ${action} word`);
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      ...
      <div className="flex justify-between items-center mb-6">
        <button className="btn btn-outline btn-sm" onClick={() => setShowStats(!showStats)}>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                {pendingWords.map((word) => (
                  <div key={word.id} className="p-6 hover:bg-gray-50">
                    <div className="mb-4">
                      ...
                      <p className="text-sm text-gray-500">Submitted {formatDate(word.createdAt)}</p>
                    </div>
                    <div className="mb-4">
                      {word.meanings?.map((meaning, index) => (
                        <div key={index} className="mb-3">
                          <p className="font-medium">
                            Definition ({meaning.language === 'en' ? 'English' : 'Hindi'}):
                          </p>
                    ...
                    <div className="flex gap-3 justify-end">
                      <button
                        onClick={() => handleActionWithComment(word.id, 'reject')}
                      ...
                      <button
                        onClick={() => handleActionWithComment(word.id, 'approve')}
        ...
        {activeTab === 'reports' && (
          <div>
            ...
            {reportsError && <div className="alert alert-error m-4"><span>{reportsError}</span></div>}
          </div>
        )}
      ...
      {showCommentModal && (
        <div className="modal modal-open">
          <div className="modal-box">
            <h3 className="font-bold text-lg mb-4">Add Comment (Optional)</h3>
            ...
            <div className="modal-action">
              ...
              <button
                className={`btn ${
                  currentAction?.action === 'approve' ? 'btn-success' : 'btn-error'
                }`}
